...
var dbs;
MongoClient.connect('mongodb://localhost/FetchApiDemoApp1', function(err, database) {
    if (err) {
      console.log('############# > server > MongoClient.connect > Error: ', err);
      return console.log(err);
    }
    dbs = database
    app.listen(process.env.PORT || 3000, function() {
      console.log('############# > server > MongoClient.connect > listening on 3000');
    });
});
...
app.get('/', function(req, res) {
  dbs.collection('myFavNoveltiesCollection').find().toArray( function(err, result) {
      	if (err) {
        	console.log('############# > server > app.get > ERROR')
        	return console.log(err)
      	}
